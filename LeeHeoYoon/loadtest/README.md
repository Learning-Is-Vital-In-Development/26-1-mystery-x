...
| 5 | Delete Files | 개별 파일 ��시 삭제 + 빈 폴더 검증 |
...
./benchmark -host http://my-storage-nginx:80 -user-id 1

# multipart 업로드 (전통 방식)
./benchmark -host http://my-storage-nginx:80 -user-id 1 -upload-mode multipart
```
...
| 플래그 | 기본값 | 설명 |
|--------|--------|------|
| `-host` | `http://my-storage-nginx:80` | 대상 서버 URL (Nginx 경유) |
